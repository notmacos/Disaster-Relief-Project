...
function addEvent(eventType, location, time, poster, description, emergency, callback) {
  const query = 'INSERT INTO events (eventType, location, time, poster, description, emergency) VALUES (?, ?, ?, ?, ?, ?)';
  db.run(query, [eventType, location, time, poster, description, emergency], function(err) {
    if (err) {
...
// Add this new function to fetch all data from the database
  ...
  db.all('SELECT name FROM sqlite_master WHERE type="table"', [], (err, tables) => {
    ...
    tables.forEach((table) => {
      db.all(`SELECT * FROM ${table.name}`, [], (err, rows) => {
        if (err) {
          console.error(`Error fetching data from ${table.name}:`, err.message);
        } else {
...
app.post('/api/events', express.json(), (req, res) => {
  const { eventType, location, time, poster, description, emergency } = req.body;
  addEvent(eventType, location, time, poster, description, emergency, (err) => {
    if (err) {
...
// Add this new API endpoint to serve all database content
app.get('/api/database', (req, res) => {
  fetchAllData((err, data) => {
    if (err) {
      res.status(500).json({ error: 'Error fetching database content' });
    } else {
      res.json(data);
    }
  });
